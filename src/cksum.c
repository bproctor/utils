...
static void do_cksum (int file, int blksize, unsigned char *buf, char *name)
{
   ...
   while ((i = read (file, buf, blksize)) > 0)
   {
      ptr = buf;
      length += i;
      while (i--)
   ...
   while (i > 0)
   {
      crc = (crc << 8) ^ crc_table[((crc >> 24) ^ i) & 0xFF];
      i >>= 8;
   }
   crc = ~crc & 0xFFFFFFFF;
...
int command_cksum (int argc, char **argv)
{
   int i;
   int in = STDIN_FILENO;
   unsigned char *buf;
   ...
   if (argc == 1)
   {
      ...
      buf = xmalloc (s.st_blksize);
      do_cksum (STDIN_FILENO, s.st_blksize, buf, NULL);
      return (EXIT_SUCCESS);
   }
   ...
   for (i = 1; i < argc; ++i)
   {
      ...
      fstat (in, &s);
      buf = xmalloc (s.st_blksize);
      do_cksum (in, s.st_blksize, buf, argv[i]);
   }
